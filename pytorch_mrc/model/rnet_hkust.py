"""
implements "HKUST R-Net"(https://github.com/HKUST-KnowComp/R-Net) with PyTorch.
There are some slight deference with hkust model:
1. We use dynamic batch and padding, rather than static
2. When encoding, we just use zero to init Bi-GRU, but hkust uses a trainable variable to init
"""
...
class RNET(BaseModel):
    def __init__(self, vocab, device,
        ...
        # Gated attention RNNs in the paper
    ...
    def forward(self, data):
        ...
        gated_att_repr = self.gated_att_bigru(encoder_context, encoder_question, context_len, question_mask)  # B*CL*2H

        # 4. Self matching attention
        self_att_repr = self.self_match_att(gated_att_repr, gated_att_repr, context_len, context_mask)  # B*CL*2H

        # 5. Pointer Network
        start_logits, end_logits = self.pointer_net(self_att_repr, encoder_question, context_mask, question_mask)
